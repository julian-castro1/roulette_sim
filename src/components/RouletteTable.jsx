...
function RouletteTable({ currentChip, chipColors }) {
  ...
  function recalculatePayouts(bets) {
    ...
    bets.map((bet) => {
      // console.log(
      //   `The bet is: $${colorToValue[bet.value]} on values ${bet.places}`
      // );
      let basePayout = 36 / bet.places.length;
      let fullPayout = colorToValue[bet.value] * basePayout;
      // console.log(
      //   `so base payout is: $${basePayout} and total payout is: ${fullPayout}`
      // );
      for (let colorVals in bet.places) {
        // console.log(`place "${bet.places[colorVals]}" in ${bet.places}`);
        numberTotals[bet.places[colorVals]] = numberTotals[
          bet.places[colorVals]
        ]
          ? numberTotals[bet.places[colorVals]] + fullPayout
          : fullPayout;
      }
    });
  ...
  const scrollLeft = window.pageXOffset || document.documentElement.scrollLeft;
    ...
    console.log(`CLICKED:  X: ${clientX}, Y: ${clientY}`)
    ...
    // offset everything relative to the actual page (not just viewport)
    left += scrollLeft;
    right += scrollLeft;
    bottom += scrollTop;
    top += scrollTop;

    console.log(`DIV:  X: ${left}, Y: ${top}`)
    ...
    function classifyClick(offsetX, offsetY, height, width, clickedKey) {
      if (clickedKey == "0" || clickedKey == "00") return "M";
    ...
    function getRelevantNumbers(clickedKey, classifiedClick) {
      ...
      // determine if the attempted bet is valid. Makes this determination by looking at the availability of surrounding numbers.
      let invalid =
        (classifiedClick == "L" && !spotClicked.left) ||
        (classifiedClick == "R" && !spotClicked.right) ||
        (classifiedClick == "T" && !spotClicked.up) ||
        (classifiedClick == "X" && !spotClicked.down) ||
        (classifiedClick == "A" && (!spotClicked.left || !spotClicked.up)) ||
        (classifiedClick == "B" && (!spotClicked.right || !spotClicked.up)) ||
        (classifiedClick == "C" && (!spotClicked.left || !spotClicked.down)) ||
        (classifiedClick == "D" && (!spotClicked.right || !spotClicked.down));
  ...
  // if the click is on the side and includes 2 numbers
      if ( ["L", "R", "X", "T"].includes(classifiedClick) && tableDef[clickedKey].class == "number" ) {
        switch (classifiedClick) {
          case "L":
            numbersClicked.push(spotClicked.left);
            chipLocation = middleLeft;
            break;
          case "R":
            numbersClicked.push(spotClicked.right);
            chipLocation = middleRight;
            break;
          case "X":
            numbersClicked.push(spotClicked.down);
            chipLocation = middleBottom;
            break;
          case "T":
            numbersClicked.push(spotClicked.up);
            chipLocation = middleTop;
            break;
        }
      } else {
        ...
        switch (classifiedClick) {
          case "A":
            numbersClicked.push(spotClicked.left);
            numbersClicked.push(spotClicked.up);
            numbersClicked.push(tableDef[spotClicked.up].left);
            chipLocation = topLeft;
            break;
          case "B":
            numbersClicked.push(spotClicked.right);
            numbersClicked.push(spotClicked.up);
            numbersClicked.push(tableDef[spotClicked.up].right);
            chipLocation = topRight;
            break;
          case "C":
            numbersClicked.push(spotClicked.left);
            numbersClicked.push(spotClicked.down);
            numbersClicked.push(tableDef[spotClicked.down].left);
            chipLocation = bottomLeft;
            break;
          case "D":
            numbersClicked.push(spotClicked.right);
            numbersClicked.push(spotClicked.down);
            numbersClicked.push(tableDef[spotClicked.down].right);
            chipLocation = bottomRight;
            break;
        }
      }
    ...
    let clickLocation = classifyClick(
      offsetX,
      offsetY,
      divHeight,
      divWidth,
    ...
    let [numbersClicked, chipLocation] = getRelevantNumbers(
      clickedKey,
      clickLocation
    );
  ...
  return (
    <MainContainer>
        ...
        {Object.keys(tableDef).map((valKey) => {
          ...
          return (
            ...
            >
              ...
              {showPayout &&
                (spot.class == "number" || spot.class == "zero") && (
                  <SpotPayout
                    showpayout={showPayout.toString()}
                    back={gridStyle.backgroundColor}
                  >
                    ${betPayouts[spot.value] ? betPayouts[spot.value] : 0}
                  </SpotPayout>
                )}
            </Spot>
          );
      ...
      <ControlButtons>
        <ShowPayoutButton
          show={showPayout.toString()}
          onClick={toggleShowPayout}
        >
          <SampleText>SHOW</SampleText>
          <SampleText> PAYOUT</SampleText>
        </ShowPayoutButton>
        <ClearBoardButton onClick={clearBoard}>CLEAR BOARD</ClearBoardButton>
        <ClearBoardButton onClick={undoBoard}>UNDO</ClearBoardButton>
        <ClearBoardButton onClick={redoBoard}>REDO</ClearBoardButton>
      </ControlButtons>
    </MainContainer>
  );
}
...
const Spot = styled.div`
  ...
  flex-direction: ${(props) =>
    props.chipclass == "third" || props.chipclass == "half" ? "row" : "column"};
  gap: ${(props) =>
    props.chipclass == "third" || props.chipclass == "half" ? ".7rem" : "0"};
